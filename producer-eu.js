...
const kafka = new Kafka({
    ...
    sasl: {
        ...
        username: process.env.CA_READ_MAIL_USERNAME_EU, password: process.env.CA_READ_MAIL_PASSWORD_KAFKA_EU,
    },
})

const producer = kafka.producer();
const consumer = kafka.consumer({ groupId: process.env.CA_READ_MAIL_TOPIC_GROUP_EU })
...
try {
    await consumer.subscribe({ topic: process.env.CA_READ_MAIL_TOPIC_KAFKA_EU, fromBeginning: true })
} catch (error) {
...
export async function writeKafka(event_data) {
    ...
    try {
        await producer.send({
            topic: process.env.CA_READ_MAIL_TOPIC_KAFKA_EU,
            messages: [
                {
